...
# Vocabulary sizes (set these according to your dataset)
movie_vocab_size = 4000   # number of movies
genre_vocab_size = 18    # number of unique genres
...
def evaluate(model, genre_model, genre_diff, split_csv, diff, device):
    ...
    for i in range(0, len(movie_seq), batch_size):
        ...
        focal_loss = FocalLoss(alpha=1, gamma=5)
...
# =============================================================================
# 7. TRAINING FUNCTION FOR ONE FOLD (with Genre Integration)
# =============================================================================
def train_fold(fold):
    print(f"\n========== Fold {fold} ==========")
    ...
    # For the movie branch, pass the movie vocabulary size.
    model = MovieTenc(args.hidden_factor, movie_vocab_size, seq_size,
    ...
    genre_model = Tenc(args.hidden_factor, genre_vocab_size, seq_size,
                        args.dropout_rate, args.diffuser_type, device)
    ...
    for epoch in range(args.epoch):
        ...
        for batch_data in train_loader:
            ...
            loss1, predicted_x = diff.p_losses(model, x_start, h, n,
                                                genres_embd=genre_predicted_x, loss_type='l2')
            predicted_items = model.decoder(predicted_x)
            focal_loss = FocalLoss(alpha=1, gamma=5)
            loss2 = focal_loss(predicted_items, target_batch)
            loss = (loss1 + loss2) / 2
...
# =============================================================================
# 8. MAIN FUNCTION (10-Fold CV or Tuning Mode)
# =============================================================================
def main():
    NUM_FOLDS = 10
    fold_val_metrics = []
    ...
    if args.tune:
        ...
        for metric in metrics:
            for value in tqdm(metric.values, desc=f"Tuning {metric.name}"):
                ...
                print(f"Hyperparameter {metric.name} = {value} gives average Val HR@5: {hr_val_avg:.4f}, "
                      f"Test HR@5: {hr_test_avg:.4f}")
