...
MERGED_DATA_DIR = "mind_processed"
...
def evaluate(model, diff, dataset_split, device):
    ...
    return {'loss': avg_loss, 'HR5': hr_list[0], 'NDCG5': ndcg_list[0],
            'HR10': hr_list[1], 'NDCG10': ndcg_list[1],
             'HR10': hr_list[2], 'NDCG10': ndcg_list[2]}
...
def train_fold(fold):
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            scheduler.step()
    os.makedirs("./models/mind", exist_ok=True)
    torch.save(model.state_dict(), f"./mind/models/genre_tenc_fold{fold}.pth")
    torch.save(diff, f"./mind/models/genre_diff_fold{fold}.pth")
    return fold_metrics
...
def main():
    ...
    if args.tune:
        ...
        os.makedirs("./mind/category", exist_ok=True)
        ...
        args.lr = best_lr
        tuning_lr_recorder.save_to_file("./mind/category/tuning_lr.json")
        ...
        tuning_optimizer_recorder.save_to_file("./mind/category/tuning_optimizer.json")
        ...
        tuning_timesteps_recorder.save_to_file("./mind/category/tuning_timesteps.json")
        ...
        tuning_dropout_recorder.save_to_file("./mind/category/tuning_dropout.json")
        ...
        tuning_l2_recorder.save_to_file("./mind/category/tuning_l2.json")
        ...
        args.eps = best_eps
        tuning_eps_recorder.save_to_file("./mind/category/tuning_eps.json")

        # Write fold metrics for tuning fold
        with open("./mind/category/fold_metrics_tune.txt", "w") as f:
            f.write("Detailed fold metrics (tuning mode, fold 1):\n")
            f.write(str(train_fold(tuning_fold)))
        print("Tuning fold metrics saved to ./mind/category/fold_metrics_tune.txt")
        ...
        with open("./mind/category/best_candidates.json", "w") as f:
            json.dump(best_candidates, f, indent=2)
        print("Best candidates saved to ./mind/category/best_candidates.json")
    ...
    print(avg_metrics)
    with open("mind/category/average_metrics.txt", "w") as f:
        f.write(str(avg_metrics))
    print("Full average metrics saved to /mind/category/average_metrics.txt")
    loss_recorder = LossRecorder(save_dir="mind/category")
    ...
    loss_recorder.save_to_file("mind/category/loss_data.json")
    avg_train, avg_test = loss_recorder.compute_average_losses()
    epochs_train = sorted(avg_train.keys())
    np.savetxt("mind/category/avg_train_loss.txt", np.array([avg_train[e] for e in epochs_train]))
    print("Average training losses saved to mind/category/avg_train_loss.txt")
    metrics_recorder = MetricsRecorder(save_dir="mind/category")
    metrics_recorder.add_fold(fm)
    metrics_recorder.save_to_file("mind/category/average_test_metrics.txt")
